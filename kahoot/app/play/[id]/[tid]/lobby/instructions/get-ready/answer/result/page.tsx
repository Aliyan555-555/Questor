...
const page = () => {
  ...
  useEffect(() =>{
    socket?.on("next_question_redirection",()=>{
      navigation.push(`/play/${quizId}/${teacherId}/lobby/instructions/get-ready`)
    })
    socket?.on("ranking_redirection_student_process",()=>{
      navigation.push(`/play/${quizId}/${teacherId}/lobby/instructions/get-ready/ranking`)
    })
  },[])
console.log(student)
  return (
    <div className="w-screen h-screen flex flex-col relative">
      ...
      <div className="flex items-center gap-4 justify-center h-[90%] w-full flex-col ">
       ...
       {decoded.resultStatus && <div className="text-2xl font-bold text-white flex gap-3 items-center">
          <p>Answer Streak</p>
          <div className="w-[24px] h-[24px] flex items-center justify-center relative">
            <span className="text-lg z-10 relative font-bold  text-white">{decoded.studentRank}</span>
            <svg
