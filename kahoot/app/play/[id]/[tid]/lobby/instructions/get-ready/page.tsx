...
console.log(student)
    ...
    socket?.on("results",(data)=>{
      ...
      navigation.push(`/play/${quizId}/${teacherId}/lobby/instructions/get-ready/answer/result?r=${encodeURIComponent(JSON.stringify(data.question))}`);
    })
    ...
    const interval = setInterval(() => {
      if ( timeCount === 0){
        ...
        // socket?.emit("submit_question_answer", {answer:null,studentId:student.student.id,question:currentQuestion,duration:30,attemptTime:30})
      }
  ...
  const handleSubmitQuestionAnswer  = (answer) => {
    socket?.emit("submit_question_answer", {answer,studentId:student.student._id,question:currentQuestion,duration:30,timeTaken:timeCount})
    ...
    <div className="w-screen h-screen flex flex-col relative">
      ...
      <div className="w-full h-[90%] flex items-center justify-center ">
       ...
       <div className="w-full flex items-center flex-col justify-center gap-5 ">
        <h3 className="text-4xl font-bold text-white">Question {questionIndex}</h3>
           <svg
