...
const GalleryModel = ({ open, close, setImage }: { open: boolean, close: () => void, setImage: (value: string) => void }) => {
    ...
    return (
        <Backdrop open={open} className='!z-[10000000000] !flex items-center justify-center'>
            <div ref={modalRef} className='flex w-[65%] h-[600px] bg-white rounded-lg shadow-lg overflow-hidden'>
...
export default React.memo(GalleryModel);

const UnsplashImages = React.memo(
    ({ searchQuery, setSearchQuery, setImage, close }: { setImage: (value: string) => void, searchQuery: string | null; setSearchQuery: (value: string) => void; close: () => void }) => {
